...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = items.merge(orders, on="orders_id").merge(products, on="product_id") 
    ...
    aggregations = aggregations = delivered.groupby("order_id").agg({
        "freight_value": "sum",
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    result_df = order_purchase_ammount_per_date["order_purchase_timestamp"].isin(holidays["date"])
