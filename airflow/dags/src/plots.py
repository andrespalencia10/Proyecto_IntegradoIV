...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.ylabel("Valor del Flete (BRL)")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df["order_purchase_timestamp"], df["order_count"], marker="o", linestyle="-")
    for holiday in df[df["holiday"] == True]["order_purchase_timestamp"]:
        plt.axvline(x=holiday, color='r', linestyle='--', alpha=0.7)
    plt.xlabel("Fecha")
    plt.ylabel("Cantidad de Pedidos")
    plt.title("Cantidad de Pedidos por Día en 2017 con Días Festivos")
